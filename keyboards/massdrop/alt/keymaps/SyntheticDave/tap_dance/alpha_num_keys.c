...
void m_finished(qk_tap_dance_state_t *state, void *user_data) {
    ...
    switch (xtap_state.state) {
        ...
        case SINGLE_TAP:
            SEND_STRING(SS_LCTL(SS_LSFT(SS_LALT(SS_TAP(X_S)))));
            break;
        // Spotify on hold
        case SINGLE_HOLD:
            SEND_STRING(SS_LCTL(SS_LSFT(SS_LGUI(SS_LALT(SS_TAP(X_M))))));
            break;
    }
}
...
void s3c_finished(qk_tap_dance_state_t *state, void *user_data) {
    ...
    switch (xtap_state.state) {
        // S3 Command
        case SINGLE_TAP:
            SEND_STRING(SS_LCTL(SS_LSFT(SS_LALT(SS_TAP(X_3)))));
            break;
        // S3 Command with last used client
        case SINGLE_HOLD:
            SEND_STRING(SS_LCTL(SS_LSFT(SS_LGUI(SS_LALT(SS_TAP(X_3))))));
            break;
    }
}
